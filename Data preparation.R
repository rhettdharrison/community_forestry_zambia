...
getwd()
list.files('../')
cfmg <- read_xlsx('../cfm_dataset_V3.xlsx',na = "NA")

cfmg <- cfmg |>
        mutate(
                province_name = as.factor(province_name),
                cfmg_name = as.factor(cfmg_name),
                fgd_participants_gender = factor(fgd_participants_gender)
        )

# Need to standardise the types of interviewee
cfmg <- cfmg |> 
        mutate(
                interviewee_role = fct_collapse(interviewee_role, 
                                                cfmg_member = c("cfmg_ordinary_member",
                                                                         "committee_member",
                                                                         "honourary_forest_officer"),
                                                trad_leader = c("traditional_leader",
                                                                       "chief_representative"),
                                                cfmg_exec_member = c("cfmg_executive_member",
                                                                          "cooperative_chairman",
                                                                          "beekeeping _chairperson"),
                                                NGO_rep =c("representative_of_ngo_associated_with_cfm",
                                                                                             "forestry_technologist"))
        )

# Need to standardise the fgd types

cfmg <- cfmg |>
        mutate(
                fgd_participants_gender = ifelse(is.na(fgd_participants_gender),
                                                 "",
                                                 paste(fgd_participants_gender)),
                fgd_participants_gender = ifelse(interview_type == "individual_interview",
                                                 NA,
                                                 paste(fgd_participants_gender)),
                fgd_type = fct_cross(interview_type, fgd_participants_gender),
                dfo = ifelse(dfo_visit_cfmg == "never_0_times","0","NA"),
                dfo = ifelse(dfo_visit_cfmg == "once_1_time","1",dfo),
                dfo = ifelse(dfo_visit_cfmg == "twice_2_times","2",dfo),
                dfo = ifelse(dfo_visit_cfmg == "thrice_3_times","3",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "More than 3 times","4",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "More than 4times","5",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "Five times","5",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "10 times",">5",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "12 times",">5",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "24 times",">5",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "36 times",">5",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "6 times",">5",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "7 times",">5",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "A lot of times I can't even count",">5",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "Comes only when BCP has a meeting","1",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "DFO only comes when there is a program","1",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "More than 5 times, but always called when there is a need.",">5",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "Only comes when the HFOs have apprehended a charcoal","1",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "Rarely involved","1",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "Several times",">5",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "She usually comes more that 5times",">5",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "Very often",">5",dfo),
                dfo = ifelse(dfo_visit_cfmg_other == "The group hasn't been working for the last 2 years and the forest officer did not visit","0",dfo),
                dfo = as.factor(dfo)
        )
...
cfmg |> group_by(province_name,cfmg_name) |>
        ...
        ggplot(aes(x = province_name, y = Obs)) +
        geom_violin() +
...
cfmg |> filter(!is.na(interviewee_gender)) |>
        group_by(interviewee_gender) |>
        summarise(
                Obs = n()
        ) |>
        gt()

# Much larger number of interviewees were male

cfmg |> filter(!is.na(interviewee_gender)) |>
        ggplot(aes(x = interviewee_gender, y = interviewee_age)) +
        geom_boxplot() +
        geom_jitter()+
...
# Median age was similar but average was higher for male, because of some much older interviewees

cfmg |> filter(!is.na(interview_type)) |> 
        filter(interview_type != "individual_interview") |>
        mutate(
                fgd_participants_gender = ifelse(is.na(fgd_participants_gender),"X",paste(fgd_participants_gender)),
                fgd_types = fct_cross(interview_type, fgd_participants_gender)
        ) |>
        group_by(fgd_types) |>
        summarise(
                Obs = n()
...
with(cfmg, table(cfmg_name, num_hh_affected_cfma))
# Data on HH affected very unreliable - same cfmg with <50 and >1000
# Just use data from CFMG_executive members
cfmg |> filter(interview_type == "cfmg_executive") |>
        group_by(num_hh_affected_cfma) |>
        summarise(
                Obs = n()
        ) |>
        gt()
...
cfmg |> filter(know_size_area_managed_cfmg...43 == 1) |>
        group_by(cfmg_name) |>
        summarise(
...
cfmg |> filter(know_size_area_managed_cfmg...43 == 1) |>
        group_by(province_name, cfmg_name) |>
        summarise(
                CFMG_size = round(mean(size_cfma),digits = 0)
        ) |>
        ungroup() |>
